export default () => {
	let knowledge = null
	let data = null
	let CLASS_TIMES = null
	let NUM_WEEKS_IN_SEMESTER = null
					...
					filter_classrooms_for_discipline(get_classrooms(), disciplineId).forEach(classroom => {
						...
						get_days_for_teacher(teacher).forEach(day => {
							...
							CLASS_TIMES.forEach(time => {
								...
								if (is_correct_schedule([...lessons, {
									day,
									time,
									classroom,
									teacher,
...
// ДОПОЛНИТЕЛЬНО - НЕТ В ОТЧЁТЕ Группа слушает столько часов дисциплины в неделю, сколько положено ей по плану
	function correct_hours_for_discipline(group, discipline, schedule) {
...
// часы на дисциплины стоят корректно (последняя проверка)
	function is_correct_hours(schedule) {
...
// предрасписание - это кусок будущего корректного расписания, где не учитываются часы на дисциплины
	...
	function get_correct_schedules() {
		console.log('get_correct_schedules', 'start')
		...
		console.log('get_correct_schedules', 'end')
		console.log(correct_schedules)
		return correct_schedules
	}

	function init(localStorage) {
		knowledge = localStorage.knowledge
		data = localStorage.data
		CLASS_TIMES = knowledge["times"]
		NUM_WEEKS_IN_SEMESTER = localStorage.NUM_WEEKS_IN_SEMESTER
	}
